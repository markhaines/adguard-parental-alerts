...
def load_state():
    try:
        ...
        return {"cursor": state.get("cursor"), "pending": state.get("pending", [])}
...
def entry_id(entry):
    ...
    return "|".join((entry.get("time", ""), question.get("name", ""), entry.get("client", "")))
...
def fetch_since(session, cursor):
    ...
    for page in range(MAX_QUERY_PAGES):
        ...
        if found_cursor or len(page_entries) < QUERY_PAGE_SIZE or not cursor:
            break
    if cursor and not found_cursor:
        raise RuntimeError(f"Query cursor was not found within {MAX_QUERY_PAGES} pages")
    return entries


def deliver_pending(state):
    for alert in list(state["pending"]):
        if not send_pushover(alert["title"], alert["message"]):
            break
        state["pending"].remove(alert)
        save_state(state)
...
def check_adguard(state):
    try:
        ...
        s.verify = False
        entries = fetch_since(s, state["cursor"])
        if state["cursor"] is None:
            ...
            return
        pending_ids = {alert["id"] for alert in state["pending"]}
        for e in reversed(entries):
            domain = e.get("question", {}).get("name", "unknown")
            eid = entry_id(e)
            if is_parental_block(e) and eid not in pending_ids:
